	...
	FVector NormalImpulse, const FHitResult& Hit)
{
	if (HasAuthority())
	{
		...
		const float ProbBounds = FMath::RandRange(0.f,1.f);
		if (AttackOwner && OtherActor != AttackOwner && ProbBounds >= 0.1)
		{
			if (Explosion)
			{
				if (SpawnedControlelr)
				{
					SpawnedControlelr->ServerSpawnParticles(Hit.Location, Explosion, FRotator::ZeroRotator);
					SpawnedControlelr->ServerMakeExplosion(Hit.Location, AttackOwner->AttackDamage);
				}
			}
			Destroy();
		}
